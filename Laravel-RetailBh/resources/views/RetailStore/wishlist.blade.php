				...
				<div id="responsive-nav">
					...
					<ul class="main-nav nav navbar-nav">
						...
						<li class="nav-item dropdown">
							<a class="nav-link dropdown-toggle" href="/index" id="navbarDropdownMenuLink" data-toggle="dropdown">Hot Deals</a>
...
<?php
 ...
 $prod=DB::table('wishlist')->select('*')->where('user_id',Session::get('idd'))->get();
		                foreach($prod as $pro){
                        $productid=$pro->p_id;
...
@else
<?php
 $prod=DB::table('wishlist')->select('*')->where('user_id',-1)->get();
		                foreach($prod as $pro){
                        $productid=$pro->p_id;
...
$sel1=DB::table('products')->select('*')->where('product_id', $productid)->get();
           ...
           <div class="section">
            ...
            ?>
             <div>
					<button class="button button3">PLACEORDER</button>
				  </div>
				  <br>
				  <br>
				  <br>
